(ns starter.sign-up.view
  (:require [starter.sign-up.form-validation :as form]
            [clojure.string :as str]
            [starter.sign-up.handler :as handler]
            [form-validator.core :as fv]
            [reagent.core :as r]
            [starter.api.paths :as api-paths]))

(swap! fv/conf #(merge % {:atom r/atom}))
(def form! (fv/init-form form/form-conf))
       ...
       [:div.col-md-12
        [:div {:class-name "form-check"}
         [:input {:type       :checkbox
                  ...
                  :on-click   #(let [checked? (.. %1 -target -checked)
                                     spec     (if checked? ::form/user-template-email-password-full
                                                           ::form/user-template-email-password)]
                                 (reset! free-trial? checked?)
                                 (swap! form! assoc :form-spec spec))
                  :id         "flexCheckDefault"}]
         ...
         [:<>
          ...
          [:div.col-md-6
           [FormControl {:name          :city
                         :default-value (-> @form! :names->value :city)
                         :placeholder   "City"
                         :on-change     on-change
                         :on-blur       on-blur
          ...
          [:div.col-md-6
           [FormControl
            {:as            :select
             :name          :country
             :default-value (-> @form! :names->value :country)
          ...
          [:div.col-md-5
           [FormControl {:name          :coupon
                         :default-value (-> @form! :names->value :coupon)
...
(defn SignUpButton []
  (let [on-submit #(when (form/form-valid? form!)
                     (handler/add-user
                       (form/form->data form!)
                       response-error!))]
